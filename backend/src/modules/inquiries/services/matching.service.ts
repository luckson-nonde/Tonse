...
export class MatchingService {
  ...
  async findLeadsForProfile(
    ...
    const params: any[] = [selector.profileId];
    let p = 1; // bound-parameter cursor

    let where = ` AND i.status = $${++p}`;
    params.push(filters.status || 'OPEN');
    ...
    const inquirySql = `
      SELECT DISTINCT i.* FROM inquiries i
      JOIN inquiry_categories ic ON ic."inquiryId" = i.id
      WHERE ic."categoryId" = ANY($2::varchar[])
      ${where}
      ORDER BY i."createdAt" DESC
      LIMIT $${++p} OFFSET $${++p}
    `;
    const countSql = `
      SELECT COUNT(DISTINCT i.id)::int AS count FROM inquiries i
      JOIN inquiry_categories ic ON ic."inquiryId" = i.id
      WHERE ic."categoryId" = ANY($2::varchar[])
      ${where}
    `;

    // Re-shape params: $1 reserved for profileId (unused by these two
    // queries — the matched ids array carries it), $2 = matchedCategoryIds.
    const queryParams = [selector.profileId, matchedCategoryIds, ...params.slice(1)];
    const dataParams = [...queryParams, limit, offset];

    const [rows, countRows] = await Promise.all([
      this.inquiryRepository.query(inquirySql, dataParams),
      this.inquiryRepository.query(countSql, queryParams),
    ]);
