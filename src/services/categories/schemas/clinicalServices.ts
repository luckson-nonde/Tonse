...
export const homeCareSchema: FieldSchema[] = [
  ...
  { name: "urgency", label: "When Should Care Start?", type: "select", required: true, options: ["Immediately", "Within a few days", "Within 2 weeks", "Flexible"], group: "Schedule & Payment" },
  { name: "preferredDateTime", label: "Preferred First Visit", type: "datetime", required: true, keepInExpress: true, helpText: "Pick the day and time for the first visit — the provider plans around it.", group: "Schedule & Payment", dependsOn: { field: "urgency", value: ["Within a few days", "Within 2 weeks"] } },
