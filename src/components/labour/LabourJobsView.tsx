      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 max-w-2xl mx-auto">
        <div className="mb-8">
          <button
            ...
            className="text-[13px] font-medium text-slate-500 hover:text-brand-dark mb-4 inline-block"
          >
            ← Back to Job Requests
          </button>
          <h2 className="text-2xl font-serif font-bold text-brand-dark">Send Proposal</h2>
        ...
        <form onSubmit={handleSendProposal} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setProposalData({ ...proposalData, availabilityDate: e.target.value })
              }
              className="w-full p-4 rounded-[16px] border border-slate-200 focus:border-[#C9973A] focus:ring-1 focus:ring-[#C9973A] outline-none transition-all"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full p-4 rounded-2xl border border-slate-200 focus:border-[#C9973A] focus:ring-1 focus:ring-[#C9973A] outline-none transition-all resize-none"
          ...
          <div className="flex gap-4 pt-4">
            <Button
              ...
              variant="outline"
              onClick={() => setSelectedJob(null)}
              className="flex-1 py-4 rounded-4xl"
            >
              Cancel
            </Button>
            <Button type="submit" className="flex-1 py-4 rounded-4xl">
              Send Proposal
            </Button>
          </div>
        </form>
    ...
    <div className="space-y-4">
      {jobRequests.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center border border-slate-100 shadow-sm">
          ...
          <h3 className="text-lg font-serif font-bold text-brand-dark mb-2">No Job Requests Yet</h3>
          <p className="text-slate-500 max-w-md mx-auto">
            There are currently no open job requests matching your skills. Check back later or
            update your profile to include more skills.
          </p>
        </div>
      ) : (
        jobRequests.map((job) => (
          <div
            key={job.id}
            className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 hover:border-[#C9973A]/30 transition-colors"
          >
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
              ...
              <div className="flex flex-col items-end gap-4 min-w-50">
                ...
                <Button onClick={() => setSelectedJob(job)} className="w-full py-3 rounded-full">
                  Send Proposal
                </Button>
              </div>
            </div>
