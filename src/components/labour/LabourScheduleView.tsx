    ...
    <div className="space-y-4">
      {schedules.length === 0 ? (
        <div className="bg-white rounded-[24px] p-12 text-center border border-slate-100 shadow-sm">
          ...
          <h3 className="text-lg font-serif font-bold text-[#1e293b] mb-2">No Confirmed Jobs Yet</h3>
        ...
        schedules.map((schedule) => (
          <div key={schedule.id} className="bg-white rounded-[24px] p-6 shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6">
            {/* Date Block */}
            <div className="flex-shrink-0 w-24 h-24 bg-[#f8f9fa] rounded-[16px] border border-slate-100 flex flex-col items-center justify-center">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                {new Date(schedule.date || schedule.createdAt).toLocaleDateString('en-US', { month: 'short' })}
              </span>
              <span className="text-3xl font-serif font-black text-[#1e293b]">
            ...
            <div className="flex-1 flex flex-col justify-center">
              <h3 className="text-lg font-serif font-bold text-[#1e293b] mb-2">
