        ...
        <div className="p-6 overflow-y-auto flex-1 scrollbar-hide">
          <div className="space-y-4">
            <div className="aspect-video bg-slate-200 rounded-2xl flex items-center justify-center relative">
              ...
              <span className="absolute bottom-4 left-4 font-bold text-white bg-black/50 px-2 py-1 rounded-lg text-xs">Video Proof</span>
            </div>
            <div className="grid grid-cols-3 gap-4">
              {[1, 2, 3].map((img) => (
                <div key={img} className="aspect-square bg-slate-200 rounded-2xl flex items-center justify-center text-slate-400 font-bold text-sm">
                  Img {img}
                </div>
              ))}
            </div>
