...
export default function InquiryDetails({ inquiry, quotes, onAction }: InquiryDetailsProps) {
  ...
  return (
    <div className="space-y-8">
      {/* Status Banner */}
      <div 
        className="p-6 rounded-[32px] flex items-center justify-between gap-4 border"
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-[#1e293b]">{statusInfo.label}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
            <h2 className="text-2xl font-serif font-black text-[#1e293b] mb-4">{inquiry.title}</h2>
            ...
            <div className="grid grid-cols-2 gap-6 pt-8 border-t border-slate-100">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-[#1e293b]">{inquiry.location}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="p-2 bg-slate-50 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-[#1e293b]">{new Date(inquiry.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {inquiry.attributes && Object.keys(inquiry.attributes).length > 0 && (
            <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
              <h3 className="text-lg font-bold text-[#1e293b] mb-6">Specifications</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {Object.entries(inquiry.attributes).map(([key, value]) => (
                  <div key={key} className="flex justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <span className="text-sm font-bold text-[#1e293b]">{String(value)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3">
              {quotes.length > 0 ? (
                quotes.map(quote => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-bold text-[#1e293b]">{quote.providerName}</span>
                      <span className="text-sm font-black text-[#C9973A]">K{quote.price.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{quote.condition}</span>
                ...
                <div className="py-8 text-center">
                  ...
                  <p className="text-xs text-slate-400 font-medium">Waiting for providers to respond...</p>
                </div>
              )}
            </div>
          </div>

          <div className="bg-[#1e293b] p-6 rounded-[32px] text-white">
            <h4 className="font-serif font-bold text-lg mb-2">Need Help?</h4>
            <p className="text-slate-400 text-sm mb-6">Our support team is available 24/7 to assist with your inquiries.</p>
