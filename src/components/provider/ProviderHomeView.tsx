...
import { 
  TrendingUp, MessageSquare, PackageOpen, MapPin, Clock, Calendar, 
      ...
      {user?.categories?.some((c: string) => c.toLowerCase().includes('equipment rental')) && (
        <div className="bg-white rounded-2xl sm:rounded-[32px] p-4 sm:p-8 shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => onTabClick('products')} className="text-[#d49b35] text-xs font-bold hover:underline">Manage All</button>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {products.length === 0 ? (
              ...
              <div key={uniqueKey('home-product', product.id, idx)} className="group cursor-pointer" onClick={() => onTabClick('products')}>
      ...
      <div>
        ...
        <div className="space-y-3">
          {leads.length === 0 ? (
            ...
            return (
              <div key={uniqueKey('home-events-lead', lead.id, idx)} className={`bg-white rounded-2xl sm:rounded-[24px] p-4 sm:p-5 shadow-sm border ${selectedInquiryIds.includes(lead.id!) ? 'border-[#d49b35] bg-[#fffaf5]' : 'border-slate-100'} flex items-start gap-4 transition-colors`}>
                {isSelectionMode && (
                  <input 
                    type="checkbox"
                ...
                <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0 w-full cursor-pointer" onClick={() => !isSelectionMode && onTabClick('leads')}>
      ...
      <div>
        ...
        <div className="space-y-3">
          {leads.length === 0 ? (
            ...
            return (
              <div key={uniqueKey('home-lead', lead.id, idx)} className={`bg-white rounded-2xl sm:rounded-[24px] p-4 sm:p-5 shadow-sm border ${selectedInquiryIds.includes(lead.id!) ? 'border-[#d49b35] bg-[#fffaf5]' : 'border-slate-100'} flex items-start gap-4 transition-colors`}>
                {isSelectionMode && (
                  <input 
                    type="checkbox"
                    checked={selectedInquiryIds.includes(lead.id!)}
