        ...
        {displayQuotes.filter(q => q.status !== 'ARCHIVED').length === 0 ? (
          <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-12 text-center border border-slate-100">
            ...
            .map((quote, idx) => {
              ...
              return (
                <div key={uniqueKey('quotes-tab-quote', quote.id, idx)} className="bg-white rounded-2xl sm:rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
                  ...
                  <div className="p-4 sm:p-6">
                        ...
                        {lead.items && lead.items.length > 0 && (
                          <div className="space-y-3">
                            ...
                            {lead.items.map((item, itemIdx) => {
                              const itemPrice = quote.itemPrices?.find(ip => ip.itemId === (item.id || `${lead.id}-item-${itemIdx}`))?.price;
                              return (
                                <div key={uniqueKey('quote-item-breakdown', undefined, `${quote.id || idx}-${itemIdx}`)} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center justify-between mt-6">
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => onToggleExpand(lead.id!)} className="flex items-center gap-1.5 text-[#d49b35] hover:underline text-[11px] font-bold">
                    ...
                    {isExpanded && lead.items && (
                      <div className="mt-4 pt-4 border-t border-slate-100 space-y-4">
                        {lead.items.map((item, itemIdx) => (
                          <div key={uniqueKey('quote-item-details', undefined, `${quote.id || idx}-${itemIdx}`)} className="bg-slate-50/50 rounded-2xl p-4 border border-slate-100">
