...
import { Truck, Clock, Check, FileText, QrCode } from 'lucide-react';
...
export default function ProviderOrdersView({
  ...
  const filteredQuotes = displayQuotes.filter(q => q.status === 'PAID' || q.status === 'AWAITING_PICKUP');
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="space-y-4 sm:space-y-6">
        {filteredQuotes.length === 0 ? (
          ...
          return (
            <div key={uniqueKey('paid-tab-quote', quote.id, idx)} className="bg-white rounded-2xl sm:rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
