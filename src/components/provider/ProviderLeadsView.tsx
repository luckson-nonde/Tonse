...
import { MapPin, Eye, ChevronDown, ChevronUp } from 'lucide-react';
  ...
  return (
    <div className="mt-6 p-6 bg-slate-50 rounded-2xl border border-slate-100">
      <div className="flex justify-between items-center mb-4">
        ...
        <span
          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${inquiry.processType === 'EXPRESS' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}
        >
      ...
      <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
        {quoteSchema.map((field) => {
          ...
          if (!field.name) {
            console.warn('Quote schema field missing name:', field);
            return null;
          }
          ...
          return (
            <div key={uniqueKey('quote-field', undefined, field.name)}>
                    ...
                    return (
                      <div className="relative">
                        <input
                          type="number"
                          value={
                            field.name === 'price' && itemPricesTotal > 0
                              ? itemPricesTotal
                        ...
                        {field.calculation &&
                          inquiry.attributes?.quantity &&
                          field.calculation === 'unit' && (
                            <span className="text-[11px] font-medium text-slate-500 mt-1.5 block">
                              × {inquiry.attributes.quantity} units = ZMW{' '}
                              {(
                                (Number(value) || 0) * Number(inquiry.attributes.quantity)
                              ).toLocaleString()}
                            </span>
                          )}
                        {field.calculation &&
                          inquiry.attributes?.rentalDuration &&
                          field.calculation === 'rate' && (
                            <span className="text-[11px] font-medium text-slate-500 mt-1.5 block">
                              × {inquiry.attributes.rentalDuration} days = ZMW{' '}
                              {(
                                (Number(value) || 0) * Number(inquiry.attributes.rentalDuration)
                              ).toLocaleString()}
                            </span>
                          )}
                      </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        {field.options.map((opt, idx) => (
                          <option key={uniqueKey('opt', undefined, `${opt}-${idx}`)} value={opt}>
                            {opt}
                          </option>
                        ))}
                      </select>
              ...
              {field.helpText && (
                <p className="text-[11px] text-slate-500 mt-1.5 italic">{field.helpText}</p>
              )}
              {errors[field.name] && (
                <p className="text-[11px] text-rose-500 mt-1.5 font-bold">
        ...
        {user?.role === 'EVENTS' && venueSpaces.length > 0 && (
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div>
              <label className="block text-[11px] font-bold text-slate-500 tracking-wider uppercase mb-2">
                Select Venue Space to Quote (Optional)
              </label>
              <Controller
                name="venueSpaceId"
                control={control}
                render={({ field: { onChange, value } }) => (
                  ...
                  >
                    ...
                    {venueSpaces.map((space, idx) => (
                      <option key={uniqueKey('venue-space', space.id, idx)} value={space.id}>
        ...
        {calculatedTotal > 0 && (
          <div className="bg-brand-white border border-[#d49b35]/20 p-4 rounded-xl mt-6">
            <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">
              Total Quote Amount
            </p>
            <p className="text-2xl font-black text-[#d49b35]">
        ...
        <div className="pt-4 flex gap-3">
          ...
          <button
            type="submit"
            className="flex-2 px-4 py-3 bg-brand-dark text-white text-sm font-bold rounded-xl hover:bg-brand-navy-dark transition-all shadow-lg shadow-slate-200"
          >
            Send Quotation
          </button>
        </div>
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mt-1">
            ...
            <span className="text-[#d49b35]">
              {user?.categories?.join(', ') || 'All Categories'}
            </span>
          </p>
        </div>
      ...
      <div className="space-y-4 sm:space-y-6">
        {leads.length === 0 ? (
          <div className="bg-white rounded-2xl sm:rounded-3xl p-10 sm:p-16 text-center border border-slate-100 flex flex-col items-center justify-center min-h-[50vh] shadow-sm">
            <img src={emptyLeadsImage} alt="No leads" className="w-48 h-48 sm:w-56 sm:h-56 object-contain opacity-90 mb-8" />
            <p className="text-slate-500 font-medium text-lg">No leads at the moment.</p>
          </div>
        ) : (
          leads.map((lead, idx) => {
            ...
            return (
            <div
              key={uniqueKey('leads-tab-lead', lead.id, idx)}
              className={`bg-white rounded-2xl sm:rounded-4xl shadow-sm border ${selectedInquiryIds.includes(lead.id!) ? 'border-[#d49b35] bg-brand-white' : 'border-slate-100'} overflow-hidden transition-colors`}
            >
                ...
                <div className="flex items-center gap-3">
                  {isSelectionMode && (
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked)
                          onSetSelectedInquiryIds([...selectedInquiryIds, lead.id!]);
                        else
                          onSetSelectedInquiryIds(
                            selectedInquiryIds.filter((id) => id !== lead.id!)
                          );
                      }}
              ...
              <div className="p-4 sm:p-6">
                <div className={quotingInquiryId === lead.id ? "flex flex-col lg:flex-row gap-8 items-start" : ""}>
                  <div className={quotingInquiryId === lead.id ? "flex-1 min-w-0" : ""}>
                    <div className="flex flex-wrap gap-2 mb-4">
                  {(lead.category || '').split(', ').map((cat: string, catIdx: number) => (
                    <span
                      key={uniqueKey('lead-cat', undefined, `${lead.id || idx}-${catIdx}`)}
                ...
                <h4 className="text-xl font-serif font-bold text-slate-900 mb-2 wrap-break-word">
                  {lead.title}
                </h4>
                <p className="text-sm text-slate-600 leading-relaxed mb-4 wrap-break-word whitespace-pre-wrap">
                  {lead.description}
                </p>

                {lead.preferences && Object.keys(lead.preferences).length > 0 && (
                  <div className="flex flex-wrap gap-3 mb-6">
                    {Object.entries(lead.preferences).map(([key, value], prefIdx) =>
                      value ? (
                        <div
                          key={uniqueKey('lead-pref', undefined, `${lead.id || idx}-${prefIdx}`)}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 rounded-xl border border-slate-100"
                        >
                        ...
                        >
                          <div className="flex gap-4">
                            {item.images && item.images.length > 0 && (
                              <div className="w-20 h-20 rounded-xl overflow-hidden border border-slate-200 shrink-0 bg-white">
                                <img
                                  src={item.images[0]}
                                  alt={item.title}
                                  className="w-full h-full object-cover"
                ...
                <div className="mt-6 pt-6 border-t border-slate-100 flex items-center justify-between">
                  ...
                  {quotingInquiryId === lead.id && (
                    <div className="w-full lg:w-[450px] lg:shrink-0 lg:sticky lg:top-8">
                      <QuoteSubmissionForm
                        inquiry={lead}
                        ...
                        user={user}
                        itemPricesTotal={
                          parsedItems.some((_: any, idx: number) => itemPrices[`${lead.id}-${idx}`])
                            ? parsedItems.reduce(
                                (sum: number, _: any, idx: number) =>
                                  sum + Number(itemPrices[`${lead.id}-${idx}`] || 0),
                                0
                              )
                            : 0
                        }
