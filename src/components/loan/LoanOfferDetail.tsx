...
export default function LoanOfferDetail({ quote, onAction }: LoanOfferDetailProps) {
  ...
  const printAgreement = () => {
    ...
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Loan Agreement ${esc(reference)}</title>
      ...
      ${d.termsDocUrl ? `<p class="tnc" style="margin-top:12px"><strong>A signed Terms &amp; Conditions document is attached to this offer</strong> — open it from the loan offer in the Nyuwe app.</p>` : ''}
      <div class="foot">
        <p><span class="status">Accepted</span></p>
        <p>This document records the loan offer terms accepted by the borrower on the Nyuwe marketplace. It is a summary for the parties' reference; disbursement and repayment are arranged directly between the lender and the borrower. Generated ${esc(date)}.</p>
      </div>
    </body></html>`);
    w.document.close();
    w.focus();
