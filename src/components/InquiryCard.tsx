...
export default function InquiryCard({ inquiry, state, quoteCount = 0, paidQuote, onAction, onDelete }: InquiryCardProps) {
  ...
  return (
    <div className={`bg-[#fdfaf6] rounded-[24px] border border-slate-200 border-t-4 ${borderColors[state]} shadow-sm overflow-hidden flex flex-col`}>
      <div className="p-6 flex-grow">
        ...
        <h3 className="text-2xl font-bold text-[#1e293b] font-serif mb-2 leading-tight">
          {inquiry.title || (inquiry.attributes && inquiry.attributes.title) || 'Untitled Inquiry'}
        </h3>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {displayAttributes.map(([key, value]) => {
            ...
            return (
              <div key={key} className={`bg-[#f4efe8] rounded-xl p-3 ${isLongText ? 'col-span-2' : 'col-span-1'}`}>
      ...
      <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-white">
        <div className="flex items-center gap-3">
          {state === 'quoted' ? (
            <div className="flex items-center gap-2 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-full">
              ...
              <span className="text-sm font-bold text-blue-700 font-sans">{quoteCount} Quotes Received</span>
            </div>
          ) : (
            <div className="text-xs font-medium text-slate-400 font-sans flex items-center gap-2">
              ...
              <span className="uppercase tracking-wider">IQR-{String(inquiry.id).substring(0, 3)}</span>
              {state === 'paid' && paidQuote && (
                <>
                  <span>·</span>
                  <span className="uppercase tracking-wider">QID-{String(paidQuote.id).substring(0, 3)}</span>
                </>
              )}
            </div>
          )}
        ...
        <Button 
          onClick={onAction}
          className={state === 'quoted' ? 'bg-[#3b82f6] hover:bg-[#2563eb] text-white font-bold rounded-xl' : 'bg-transparent border border-slate-200 hover:bg-slate-50 font-bold rounded-xl text-[#1e293b]'}
        >
