  ...
  const trendingPosts =
    useLiveQuery(async () => {
      ...
      return sortedItems.map((item, index) => ({
        ...
        image:
          shop.proofPhotos[index % shop.proofPhotos.length] ||
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2 bg-[#d49b35]/10 px-3 py-1.5 rounded-xl border border-[#d49b35]/20">
          ...
          <span className="text-xs font-bold text-[#b3822c] uppercase tracking-wider">
            Trending
          </span>
        </div>
      </div>
      ...
      {/* Shop Summary Card */}
      <div className="bg-white rounded-4xl p-6 shadow-sm border border-slate-100 mb-8 flex items-center gap-6">
        <div className="w-20 h-20 rounded-2xl bg-slate-900 flex items-center justify-center text-white font-black text-xl shadow-lg shrink-0">
          {shop.logo ? (
            <img
              src={shop.logo}
              alt={shop.name}
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
          {trendingPosts.length > 0 ? (
            trendingPosts.map((post) => (
              <div
                key={post.title}
                className="shrink-0 w-64 bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-md transition-all group cursor-pointer"
                onClick={() =>
                  navigate(`/buyer/create-inquiry?prefill=${encodeURIComponent(post.title)}`)
                }
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {shopProducts && shopProducts.length > 0 ? (
            shopProducts.map((product) => (
              <div
                key={product.id}
                className="bg-white rounded-4xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-md hover:border-[#d49b35]/30 transition-all group cursor-pointer flex flex-col"
                onClick={() =>
                  navigate(`/buyer/create-inquiry?prefill=${encodeURIComponent(product.name)}`)
                }
              >
                {/* Product Image */}
            ...
            <div className="col-span-full bg-white rounded-4xl p-12 text-center border border-dashed border-slate-200">
      ...
      {/* Call to Action */}
      <div className="mt-12 bg-slate-900 rounded-4xl p-8 text-center text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-300 text-sm mb-8 max-w-xs mx-auto">
            Send a custom inquiry to {shop.name} and get a personalized quote within minutes.
          </p>
          <button
