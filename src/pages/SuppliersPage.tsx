  ...
  return (
    <div className="flex flex-col space-y-8 max-w-3xl mx-auto pb-24 px-4 pt-2">
      ...
      <div className="flex gap-3">
        ...
        <button className="w-14 h-13.5 bg-white rounded-2xl shadow-sm border border-slate-200 flex items-center justify-center text-slate-400 hover:bg-slate-50 hover:text-[#1a1612] hover:border-slate-300 transition-all shrink-0">
      ...
      <div className="space-y-8">
        {shops.length === 0 ? (
          ...
          shops.map((shop) => (
            <div
              key={shop.id}
              className="bg-white rounded-2xl sm:rounded-4xl overflow-hidden shadow-sm border border-slate-200 transition-all duration-300 hover:shadow-xl hover:shadow-black/5 hover:border-[#d49b35]/30 group relative cursor-pointer"
              ...
              <div className="px-5 pt-8 pb-6 sm:px-6 sm:pt-10 sm:pb-6 relative flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="text-xl sm:text-2xl font-serif font-bold text-[#1a1612] line-clamp-1 group-hover:text-[#d49b35] transition-colors">
                      {shop.name}
                    </h3>
                    {shop.isVerified && (
                      <div
                        className="bg-emerald-50 text-emerald-600 p-1 rounded-full border border-emerald-100 shrink-0"
                        title="Verified Shop"
                      >
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-slate-500">
                    <span className="font-bold text-[#d49b35] text-[10px] sm:text-xs uppercase tracking-widest shrink-0">
                      {shop.category}
                    </span>
