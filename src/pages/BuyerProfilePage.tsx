...
export default function BuyerProfilePage() {
  ...
  const renderIdentitySection = (section: ProfileSection) => {
    ...
    return (
      <div
        key={section.title}
        className="bg-white border border-[#f1f5f9] rounded-3xl p-8 shadow-sm flex flex-col md:flex-row items-center md:items-start gap-8 relative overflow-hidden"
      >
        ...
        <div className="flex flex-col items-center gap-4 relative z-10">
          <div className="w-25 h-25 rounded-full bg-brand-dark flex items-center justify-center border-[6px] border-[#fdfaf6] shadow-xl overflow-hidden group relative">
            {user?.logo ? (
              <img
                src={user.logo}
                alt="Profile"
          ...
          <div className="flex flex-col items-center">
            <span className="text-[10px] font-sans font-bold text-[#C9973A] uppercase tracking-[0.2em]">
              Member ID
            </span>
            <span className="text-xs font-mono font-medium text-brand-dark">
              #TN-{user?.id?.toString().substring(0, 6).toUpperCase()}
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 text-center md:text-left relative z-10">
          <div className="flex flex-col md:flex-row md:items-center gap-4 mb-3">
            <h2 className="text-[28px] font-serif font-bold text-brand-dark tracking-tight">
              {user?.name || 'Luckson C Nonde'}
            </h2>
            <div className="flex items-center gap-2 mx-auto md:mx-0">
              ...
              {user?.verificationStatus === 'VERIFIED' && (
                <div
                  className="w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center text-white shadow-sm"
          ...
          <p className="text-[14px] font-sans text-[#64748b] mb-8 max-w-md leading-relaxed">
            {section.description ||
              'Welcome to your TONSE professional account. Manage your identity and preferences below.'}
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
            ...
            <button
              onClick={() => logout()}
              className="px-8 py-3 border border-[#f1f5f9] text-red-500 rounded-full text-sm font-sans font-medium hover:bg-rose-50 transition-colors flex items-center gap-2"
            >
    ...
    return (
      <div key={section.title} className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <h3 className="text-sm font-serif font-bold text-brand-dark uppercase tracking-[0.15em] px-4 py-1 bg-[#fdfaf6] border border-[#f1f5f9] rounded-full">
            {section.title}
          </h3>
          <div className="h-px flex-1 bg-[#f1f5f9]" />
        </div>

        <div className="bg-white border border-[#f1f5f9] rounded-3xl p-8 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-8">
            {section.fields
              ?.filter((f) => f.name !== 'logo')
              .map((field, fIdx) => (
                <div key={fIdx} className="group">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-[#fdfaf6] border border-[#f1f5f9] flex items-center justify-center text-brand-dark/40">
                      {getFieldIcon(field.name)}
                    </div>
                    <span className="text-[15px] font-sans font-medium text-brand-dark">
          ...
          <div className="max-w-xl space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full h-12 px-4 bg-[#fffef9] border border-[#e8e0d0] rounded-xl text-sm font-sans text-brand-dark focus:outline-none focus:border-[#C9973A] transition-colors"
                    placeholder="••••••••"
                  />
                  <button
                    onClick={() => setShowPasswords((p) => ({ ...p, new: !p.new }))}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-[#9ca3af] hover:text-[#C9973A]"
                  >
                    {showPasswords.new ? (
            ...
            <div className="flex items-center justify-between pt-4">
              ...
              <button
                ...
                className="px-8 py-3 bg-brand-dark text-white rounded-full text-[13px] font-sans font-bold hover:bg-black transition-all shadow-md disabled:opacity-50 flex items-center gap-2"
              >
        ...
        <div className="bg-white border border-[#f1f5f9] rounded-2xl overflow-hidden shadow-sm">
          <div className="p-8 border-b border-[#f1f5f9]">
            <p className="text-[13px] text-[#64748b] leading-relaxed">
              {section.description ||
                'Choose how you want to stay informed about your activity on the platform.'}
            </p>
          </div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-20">
      ...
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-serif font-bold text-brand-dark leading-tight tracking-tight">
            Account Settings
          </h1>
          <p className="text-sm font-sans text-slate-500 mt-1">
            Refine your professional presence and security
          </p>
        </div>
        <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-[#fdfaf6] border border-[#f1f5f9] rounded-full">
          <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
          <span className="text-xs font-sans font-bold text-brand-dark/60 uppercase tracking-widest">
            System Active
          </span>
        </div>
      </div>
      ...
      <div className="space-y-12">
        {schema.sections.map((section) => {
          switch (section.type) {
            case 'identity':
              return renderIdentitySection(section);
            case 'fields':
              return renderFieldsSection(section);
            case 'security':
              return renderSecuritySection(section);
            case 'notifications':
              return renderNotificationsSection(section);
            default:
              return null;
          }
        })}
              ...
              <div className="p-8 pb-4 border-b border-[#f1f5f9] flex items-center justify-between shrink-0">
                <div>
                  ...
                  <p className="text-[12px] text-[#64748b]">
                    Update your personal or business details
                  </p>
                </div>
                <button
                  onClick={() => setIsEditModalOpen(false)}
                  className="w-10 h-10 rounded-full bg-[#f1f5f9] flex items-center justify-center text-[#9ca3af] hover:text-brand-dark transition-all"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
...
function NotificationToggle({
  ...
  return (
    <div className="p-6 flex items-center justify-between hover:bg-[#fdfaf6] transition-colors group">
      <div className="flex items-center gap-4">
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${active ? 'bg-[#C9973A]/10 text-[#C9973A]' : 'bg-[#f1f5f9] text-[#9ca3af] group-hover:bg-white'}`}
        >
          {icon}
        </div>
        <div>
          <h4 className="text-base font-sans font-bold text-brand-dark leading-tight">{label}</h4>
          <p className="text-xs font-sans text-[#9ca3af] mt-0.5">{subLabel}</p>
        </div>
      </div>

      <button
        onClick={onToggle}
        className={`relative w-13 h-7 rounded-full transition-all duration-300 shrink-0 focus:outline-none border-2 ${active ? 'bg-[#C9973A] border-[#C9973A]' : 'bg-[#e8e0d0] border-[#e8e0d0]'}`}
      >
        <div
          className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow-md transition-transform duration-300 ${active ? 'translate-x-6' : 'translate-x-0'}`}
        />
      </button>
    </div>
  );
