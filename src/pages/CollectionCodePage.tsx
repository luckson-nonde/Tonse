...
export default function CollectionCodePage() {
  ...
  const quoteData = useLiveQuery(async () => {
    if (!id) return null;
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden relative">
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-slate-500 text-sm mb-8">
              Present this QR code or PIN to the seller or shop attendant when picking up your
              parcel in person.
            </p>
            ...
            <div className="flex justify-center mb-8">
              <div className="bg-white p-4 rounded-3xl border-2 border-slate-100 shadow-sm">
                <QRCodeSVG
                  value={`PROQUOTE-COLLECT-QT-${quoteData.id}-${quoteData.collectionCode}`}
                  size={200}
                />
              </div>
            </div>
          ...
          <div className="bg-[#fdf6e9]/50 p-6 border-t border-[#d49b35]/20">
            <div className="flex items-start gap-3">
              <ShieldCheck className="w-5 h-5 text-[#d49b35] shrink-0 mt-0.5" />
              <p className="text-sm text-slate-600 leading-relaxed">
                <strong className="text-slate-900">Secure Escrow:</strong> Scanning this code
                completes the transaction. The funds currently held in escrow will automatically be
                queued for release to the seller.
              </p>
            </div>
          </div>
        </div>
