...
export default function ArchivedQuotesPage() {
  ...
  const archivedQuotes = useLiveQuery(
    () => {
      if (!user) return [];
      return db.quotes
        .where('status')
      ...
      <div className="space-y-4">
        {archivedQuotes.length === 0 ? (
          ...
          archivedQuotes.map((quote) => (
            <div key={quote.id} className="bg-white rounded-[32px] p-6 shadow-sm border border-slate-100">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="text-right">
                  <p className="text-lg font-black text-slate-900">ZMW {quote.price.toLocaleString()}</p>
                </div>
              </div>
