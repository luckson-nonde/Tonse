    ...
    <div className="space-y-6">
      ...
      {(isAdding || isEditing) && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[32px] w-full max-w-md max-h-[90vh] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col">
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
              <h3 className="text-xl font-black text-slate-900">
                {isEditing 
                  ? (user?.role === 'EVENTS' ? 'Edit Equipment' : 'Edit Product')
                  : (user?.role === 'EVENTS' ? 'Add New Equipment' : 'Add New Product')}
              </h3>
              <button onClick={() => { setIsAdding(false); setIsEditing(false); setEditingProductId(null); setFormData({ name: '', price: '', stock: '', description: '', category: '', image: '' }); }} className="p-2 hover:bg-slate-200 rounded-full transition-colors">
            ...
            <div className="overflow-y-auto flex-1">
              <form onSubmit={handleAddProduct} className="p-8 space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{user?.role === 'EVENTS' ? 'Equipment Name' : 'Product Name'}</label>
                  <input 
                    ...
                    placeholder={user?.role === 'EVENTS' ? "e.g. Professional Sound System" : "e.g. Minimalist Smart Watch"}
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Price (ZMW)</label>
                    <input 
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{user?.role === 'EVENTS' ? 'Stock Quantity' : 'Quantity'}</label>
                    <input 
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{user?.role === 'EVENTS' ? 'Equipment Description' : 'Description'}</label>
                  <textarea 
                    ...
                    placeholder={user?.role === 'EVENTS' ? "Describe the equipment..." : "Describe your product..."} rows={3}
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{user?.role === 'EVENTS' ? 'Equipment Image' : 'Product Image'}</label>
                  <div 
                ...
                >
                  {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : (isEditing ? <Edit2 className="w-5 h-5" /> : <Plus className="w-5 h-5" />)}
                  {isEditing 
                    ? (user?.role === 'EVENTS' ? 'Update Equipment' : 'Update Product')
                    : (user?.role === 'EVENTS' ? 'List Equipment' : 'List Product')}
                </button>
              </form>
            </div>
          </div>
          ...
          <div key={product.id} className="bg-white rounded-[24px] p-4 flex items-center gap-4 shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-[#f1f5f9] group hover:shadow-md transition-all">
            ...
            <div className="flex-1 min-w-0 py-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <div className={`w-1.5 h-1.5 rounded-full ${product.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
              </div>
              <h4 className="font-bold text-slate-900 text-sm sm:text-base truncate">{product.name}</h4>
              <div className="flex items-center gap-3 mt-1">
                <p className="text-[#C9973A] font-black text-sm sm:text-base">ZMW {product.price.toLocaleString()}</p>
      ...
      {deletingProductId && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[32px] w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 p-8 text-center">
            <div className="w-16 h-16 bg-rose-100 rounded-full flex items-center justify-center mx-auto mb-6">
              <Trash2 className="w-8 h-8 text-rose-600" />
            </div>
            <h3 className="text-xl font-black text-slate-900 mb-2">Delete Item?</h3>
            <p className="text-slate-500 mb-8">Are you sure you want to remove this item from your inventory? This action cannot be undone.</p>
